...
namespace hev_settings {
  ...
  bool reset_timers_requested = false;  // Flag to request timer reset from menu
...
public:
  ...
  void Clash(bool stab, float strength) override {
    ...
    if (hev_settings::clash_damage_enabled) {
      ...
      if (!hev_settings::combat_mode && health_ != 0) {
        ...
        // 5. Morphine (queued last, ONLY IF major injury was played)
        if (queue_morphine_) {
  ...
  void HazardDecrease() {
    ...
    if (!timer_hazard_delay_.running()) {
      ...
      // For hazard damage, we should trigger armor compromised and health alerts if needed (unless dead)
...
// Volume Menu
  ...
  // Main Loop
  void Loop() override {
    // Check if timer reset was requested from settings menu
