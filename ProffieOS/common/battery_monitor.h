...
// Number of seconds to average battery readings over
#define BATTERY_AVERAGING_SECONDS 60
...
BatteryMonitor() : reader_(batteryLevelPin,
  ...
  // Returns the averaged battery voltage
  float battery() const {
    return averaged_voltage_;
  }
...
protected:
  void Setup() override {
    ...
    // Since buffer is pre-filled, initialize sum to match
    samples_sum_ = initial_voltage * BATTERY_SAMPLE_SIZE;
    SetPinHigh(false);
  }
  void Loop() override {
    ...
    while (true) {
      ...
      // Update running sum by replacing old value with new value
      ...
      // Calculate average using running sum (buffer always full)
      averaged_voltage_ = samples_sum_ / BATTERY_SAMPLE_SIZE;
