...
Also, a large variety of FREE character Voicepacks available here:
...
EFFECT2(trloop, trloop);                                                                                        // BC personal only ATM

// try updated thrust?
// #ifdef COLLECT_THRUST_TELEMETRY
// struct ThrustTelemetry {
//   ThrustTelemetry() : timestamp_millis(0), axial_mss(0.0f), transverse_mss(0.0f), velocity_estimate(0.0f), swing_speed(0.0f) {}

//   ThrustTelemetry(uint32_t t, float axial, float transverse, float ve, float swing)
//   : timestamp_millis(t), axial_mss(axial), transverse_mss(transverse), velocity_estimate(ve), swing_speed(swing) {}

//   uint32_t timestamp_millis;
//   float axial_mss;
//   float transverse_mss;
...
//   float dt = min(0.1f,(current_timestamp - velocity_estimate_last_update_timestamp_) * 0.001f);
...
// #ifdef COLLECT_THRUST_TELEMETRY
//      if (millis() != thrust_telemetry_last_collection_timestamp_)
//      {
//        thrust_telemetry_last_collection_timestamp_ = millis();
//        ThrustTelemetry telemetry_data(millis(),mss.x,sqrt(mss.y*mss.y+mss.z*mss.z), velocity_estimate_, fusor.swing_speed());
//        thrust_telemetry_buffer_[thrust_telemetry_buffer_index_] = telemetry_data; 
//        thrust_telemetry_buffer_index_ = (thrust_telemetry_buffer_index_ + 1) % THRUST_TELEMETRY_BUFFER_SIZE;

//        if (collecting_thrust_telemetry_)
//        {
//          thrust_telemetry_collections_left_ -= 1;
//          if (thrust_telemetry_collections_left_ <= 0)
//          {
//      dumpThrustTelemetry();
//            collecting_thrust_telemetry_ = false;
//          }
//        }
//      }
// #endif
...
// fett263 newer settings (too difficult to trigger?)
...
//     if (sqrt(mss.y * mss.y + mss.z * mss.z) < 15.0 &&
//       velocity_estimate_ > 1.7f &&
//       mss.x < -5 &&
//       fusor.swing_speed() < 130) {
//         uint32_t thrust_duration = millis() - thrust_begin_millis_; 
//         if (thrust_duration < 300 && thrust_duration > 60) {
...
void SoundLoop() {                                                         // BC personal only ATM
  if (wav_player) {
...
// 2nd(third) way - was fantastic but better above:
...
// ORIGINAL WAY
  ...
  bool Event2(enum BUTTON button, EVENT event, uint32_t modifiers) override {
    // Prevent any button actions until boot is finished playing.
...
// Events that need to be handled regardless of what other buttons are pressed.
  ...
  //   switch (effect) {
  //     case EFFECT_QUOTE:  // fal through to force. used to be for getting lengths for overlap delay timer
  //     case EFFECT_FORCE:
  //       return;
  //    }
  // }  // SB_Effect2
...
// // try updated thrust?
// #ifdef COLLECT_THRUST_TELEMETRY
//   void dumpFloat(float f, char *write_buffer) {
//     bool is_positive = f > 0.0f;
//     f = fabs(f);

//     int val_exp;
//     float mantissa = frexp(f, &val_exp);

//     // convert val_exp to base 10 and round
//     int val_exp10 = static_cast<int>(val_exp * log10(2));
//     float mantissa10 = f / pow(10, val_exp10);

//     // get integer and 4 decimal digits of the mantissa
//     strcpy(write_buffer, is_positive ? "+" : "-");
    
//     float mant_remaining = mantissa10;
//     for (int digitix = 0; digitix < 5; digitix++) {
//       float mant_whole = trunc(mant_remaining);
//       mant_remaining = (mant_remaining - mant_whole) * 10;
//       int mant_int = static_cast<int>(mant_whole);
//       if (digitix == 0) {
//         sprintf(write_buffer + strlen(write_buffer), "%d.", mant_int);
//       } else {
//         sprintf(write_buffer + strlen(write_buffer), "%d", mant_int);
//       }
//     }
//     sprintf(write_buffer + strlen(write_buffer), "e%d ", val_exp10);
//   }

//   void dumpThrustTelemetry() {
//     char filename_buffer[100];
//     strcpy(filename_buffer,"thrustTL");
//     itoa(dumpfile_index_, filename_buffer+strlen(filename_buffer),10);
//     strcat(filename_buffer,".dat");
//     PathHelper dumpfile_path(current_directory, filename_buffer);
//     LOCK_SD(true);
//     LSFS::Remove(dumpfile_path);
//     FileReader file;
//     file.Create(dumpfile_path);
//     // the current index points to the oldest data point
//     char write_buffer[200];
//     for (int writeix=0; writeix < THRUST_TELEMETRY_BUFFER_SIZE; writeix++) {
//       unsigned int lookup_index = (thrust_telemetry_buffer_index_ + writeix) % THRUST_TELEMETRY_BUFFER_SIZE;
//       ThrustTelemetry &v = thrust_telemetry_buffer_[lookup_index];
//       itoa(v.timestamp_millis, write_buffer,10);
//       strcat(write_buffer," ");
//       dumpFloat(v.axial_mss, write_buffer+strlen(write_buffer));
//       strcat(write_buffer," ");
//       dumpFloat(v.transverse_mss, write_buffer+strlen(write_buffer));
//       strcat(write_buffer," ");
//       dumpFloat(v.velocity_estimate, write_buffer+strlen(write_buffer));
//       strcat(write_buffer," ");
//       dumpFloat(v.swing_speed, write_buffer+strlen(write_buffer));
//       strcat(write_buffer,"\n");
...
//     // advance to the next file index
//     dumpfile_index_ = (dumpfile_index_ + 1) % 10;
//   }
// #endif  // COLLECT_THRUST_TELEMETRY
...
// // try updated thrust?
//   float velocity_estimate_ = 0;
//   uint32_t velocity_estimate_last_update_timestamp_ = 0;

// #ifdef COLLECT_THRUST_TELEMETRY
//   constexpr static const int THRUST_TELEMETRY_BUFFER_SIZE = 300;
//   constexpr static const int THRUST_COLLECTIONS_POST_EVENT = 300;

//   void start_telemetry_collection()
//   {
//     collecting_thrust_telemetry_ = true;
//     thrust_telemetry_collections_left_ = THRUST_COLLECTIONS_POST_EVENT;
//   }

//   bool collecting_thrust_telemetry_ = false;
//   int thrust_telemetry_collections_left_ = 0;
//   uint32_t thrust_telemetry_last_collection_timestamp_ = 0;
//   int dumpfile_index_ = 0;
//   unsigned int thrust_telemetry_buffer_index_ = 0;
//   ThrustTelemetry thrust_telemetry_buffer_[THRUST_TELEMETRY_BUFFER_SIZE];
// #endif  // COLLECT_THRUST_TELEMETRY
...
#endif // PROPS_SABER_BC_BUTTONS_H

/*
BC Personal Notes:
// revamped saved twists directly in DetectTwist() override

- set specific sound files use : SFX_EFFECT.Select(N); where EFFECT is out, in, preon etc... and number is file number offset by -1.
-1 is random.
ie: SFX_quote.Select(-1);  // plays random
ie: SFX_quote.Select(2);  // plays file quote01.wav
use just before PlayPolyphonic or PlayCommon.



// alternate degrees of rotation storage
//   M_PI / 60      // 3 degrees
//   M_PI / 36      // 5 degrees
//   M_PI / 18      // 10 degrees
//   M_PI / 9       // 20 degrees
...
//   2 * M_PI / 9   // 40 degrees
//   M_PI / 4       // 45 degrees
//   5 * M_PI / 18  // 50 degrees
//   M_PI / 3       // 60 degrees
//   7 * M_PI / 18  // 70 degrees
//   4 * M_PI / 9   // 80 degrees
//   M_PI / 2       // 90 degrees
  // or use this helper function and express everything as `Degrees(30)`
  // constexpr double Degrees(double degrees) {
//     return degrees * (M_PI / 180.0);
// }



                                                                  // this is from when trying to get a previous IN sound...??
                                                                  // float in_length_;
                                                                  // void SB_Effect2(EffectType effect, float location) override) {
                                                                  //    if (effect == EFFECT_RETRACTION) {
                                                                  //        // The lengh of the IN effect is available here.
                                                                  //        in_length_ = SaberBase::sound_length;
                                                                  //    }
                                                                  // }
                                                                  // void SB_Effect(EffectType effect, float location) override {
                                                                  //  switch(effect) {
                                                                  //    case EFFECT_USER4:
                                                                  //       if (skip_because_reasons) {
                                                                  //         // Use the length of the previously started "in" effect.
                                                                  //         SaberBase::sound_length = in_length_;
                                                                  //       } else {
                                                                  ...
                                                                  // }
                                                                  //
                                                                  // Ah, EFFECT_RETRACTION is a virtual effect that only exists in blades.
                                                                  // We have to capture the length in SB_Off2() instead, something like:
                                                                  //
...
-----------------
// skip a sound by temporarily setting it's volume to ZERO! lol

int saved_volume = SFX_font.GetVolume();
SFX_font.SetVolume(0);
SaberBase::DoNewFont();
...
// is some sound playing? wait until it's done:
...
// - or feed it a sound from somewhere like
...
void Loop() {
    if (!IsSoundPlaying(&SFX_mute)) {
    PVLOG_NORMAL << "hello world\n";
  }
}
...
what about something like this, with no font.config variables, just 1.0 as 100% of the time elapsed?
...
*********** for oneshots, JUST DO THIS BEFORE THE CALL TO PLAY IT      if (GetWavPlayerPlaying(&SFX_soundToNotOverlap)) return; *********



// Old Volume Menu code, keeping this for nostalgia.
// // Volume Menu
//   void VolumeMenu() {
